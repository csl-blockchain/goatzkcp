...
template Enc(nPairs) {
    ...
    var poseidonCount = (nPairs*2-1)\4;
    var rest = (nPairs*2-1)%4;
    ...
    if(rest!=0){
        ...
        for(var i=0;i<rest;i++){
            last.inputs[i+1] <== CT[poseidonCount*3+1+i];
        }
        Out <== last.out;
    } else {
...
// component main {public [Keys]} = CiminionEnc(70);